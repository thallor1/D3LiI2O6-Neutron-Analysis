...
import re
import CifFile
#Uses a python library for cif file import
...
class Material:
	...
	def gen_unit_cell_positions(self):
		...
		for position in coords:
			...
			for j in range(len(symm_ops)):
				...
				occ = occupancy
				pos = [round(x_pos,3),round(y_pos,3),round(z_pos,3),occ]
	...
	def gen_reflection_list(self,max_tau=20,maxQmag=1e10,b_dict=False):
		...
		#Imported from the NIST site so this is in femtometers, for result to be barn multiply by
		b_array=b_array*10.0
